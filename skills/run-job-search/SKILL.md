...
Read them completely before searching. Read `CAREER_HATS.md` when present so queries can use distinct credible positionings without blending them into one generic identity. If a required input is absent, ask for its location. Use `$curate-job-search-directories` when the source list itself must be created or materially refreshed.
