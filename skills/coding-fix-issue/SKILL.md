...
description: "Fix a clear issue with the smallest effective code change, local runtime evidence, regression tests, verification, and automatic coding-autonomous-execute escalation when bounded repeated repair is needed. Use when the user asks to debug/fix an existing problem."
metadata:
...
5) Red phase (test first)
   ...
   - If the issue is attached to a `FEATURE_DIR`, make sure `PROOF.md` covers the regression or use `coding-proof-author` to repair it.
