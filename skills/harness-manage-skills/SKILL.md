---
name: harness-manage-skills
description: Inspect, add, remove, refresh, or reconcile skills.toml dependencies. Use for skill inventory changes, audits, synchronization, machine bootstrap, and safely cloning or fast-forwarding the dot-codex configuration checkout.
---
...
- Refresh or clone the dot-codex configuration checkout: read [Pull config](references/pull-config.md) and use its bundled helper. A checkout refresh does not automatically reconcile or install skill dependencies; run inventory synchronization only when requested.
