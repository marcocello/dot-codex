...
description: Execute a feature queue, one feature, or a stubborn issue fix through a Codex Goal with explicit verification, evaluator judgment, bounded repair, and queue progress updates. Use when the user asks for autonomous execution, keep-going-until-done behavior, queue completion, or repeated repair after coding-feature-execute or coding-fix-issue.
---
...
feature, ensure acceptance exists, run coding-feature-execute, run $HOME/.codex/scripts/gate, run
$HOME/.codex/scripts/acceptance --feature FEATURE_DIR, run coding-feature-evaluator, repair bounded
...
coding-feature-evaluator PASS. Preserve the repo architecture and stop as BLOCKED if the same blocker
repeats three times.
```
...
5. Use `coding-acceptance-author` if acceptance coverage is missing.
...
- Keep a concise handoff with `Skills used:`, Goal, changed files, checks run, evaluator result,
  queue status, blockers, and why the Goal was completed or left blocked.
