...
The human defines the target: a feature spec (`features/<id>/feature.yaml`), constraints, and what “done” means at a high level. When explicitly prompted (e.g. via `create-acceptance`), Codex translates the acceptance criteria in `feature.yaml` into executable black-box tests under the feature folder. Those tests become the concrete definition of success.
...
Determinism comes from two scripts. These scripts decide success.
    ...
    `$HOME/.codex/scripts/acceptance --feature <dir>`  runs feature-scoped black-box tests derived from `feature.yaml`.
