...
3) Keep checks black-box
   ...
   - For API-backed behavior, start or target the real service and call the real HTTP endpoint
     with a normal HTTP client.
   - For non-API backend behavior, trigger public commands, workers, scheduled jobs, files, queues,
