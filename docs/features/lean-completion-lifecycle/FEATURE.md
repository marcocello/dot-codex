...
- The preflight performs one bounded challenge of intent, authority and state transitions, affected consumers, central false-green pressure, and feature cohesion. It returns `CLEAR`, `FINDINGS`, or `NEED_INPUT` with all supported material findings from that pass and no numeric quota.
