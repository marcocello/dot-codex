...
- `sync` verifies owned entries, resolves Git sources at current `HEAD`, materializes missing or stale third-party skills, installs missing declared plugins with `codex plugin add`, and fails on unavailable providers or enablement drift.
...
- URL sources may materialize one `SKILL.md`; Git sources may materialize one skill directory.
