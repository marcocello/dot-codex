...
class Acceptance(unittest.TestCase):
    def setUp(self):
        ...
        self.reset()
    def reset(self, mode=''):
        self.state.write_text(json.dumps({'mode':mode,'tasks':{f't{i}':{'id':f't{i}','name':f'Task {i}','description':'Keep me','team_id':'7','list':{'id':'101'},'custom_fields':[{'id':fid} for fid in ('f-text','f-number','f-drop','f-unsupported')]} for i in range(1,4)},'requests':[]}))
