...
description: Fix a clear issue with the smallest effective code change, regression tests, verification, and automatic coding-autonomous-execute escalation when bounded repeated repair is needed. Use when the user asks to debug/fix an existing problem.
metadata:
...
7) Finalize safely
   - Use `coding-feature-evaluator` when the issue fix is attached to a `FEATURE_DIR`, broad behavior
...
8) Automatically use `coding-autonomous-execute` for repeated repair
   ...
   - Use `coding-feature-evaluator` after each repair pass when a feature contract or user-visible workflow
     is in scope.
