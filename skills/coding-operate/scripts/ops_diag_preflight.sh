...
if have docker; then
  ...
  docker_cmd="${docker_cmd}docker ps --all --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}'"
...
if have az; then
  ...
  if [ -n "$resource_group" ]; then
    ...
    run "Azure resource group" "az group show -n '$resource_group' --query '$group_query' -o table"
  fi
  if [ -n "$resource_group" ] && [ -n "$static_web_app" ]; then
    swa_query="{name:name, defaultHostname:defaultHostname, branch:branch, sku:sku.name}"
    run "Azure Static Web App" "az staticwebapp show -g '$resource_group' -n '$static_web_app' --query '$swa_query' -o table"
    run "Static Web App environments" "az staticwebapp environment list -g '$resource_group' -n '$static_web_app' -o table"
  fi
else
  section "Azure CLI"
  echo "az not found"
...
if have kubectl; then
  ...
  if [ "$all_namespaces" = "true" ]; then
    ...
    run "Kubernetes namespace resources" "kubectl get deploy,rs,pod,svc,ingress -n '$namespace' -o wide"
    run "Kubernetes namespace events" "kubectl get events -n '$namespace' --sort-by=.lastTimestamp | tail -50"
  else
